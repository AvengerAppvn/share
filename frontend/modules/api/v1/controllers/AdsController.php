...
class AdsController extends ActiveController {
	...
	public function actionView() {
		...
		return array(
			...
			'is_shared'          => AdsShare::find()->where( [ 'ads_id'  => $advertise->id,
			                                                   'user_id' => $user->id
			] )->exists() ? 1 : 0,
	...
	public function actionChangeAvatar() {
		...
		if ( $advertise->logs ) {
			$logs = json_decode( $advertise->logs );
		} else {
			$logs = array();
		}

		$logs[] = array(
			time() => array( 'action' => 'compare', 'facebook_id' => $fbid )
		);

		if ( $compare > 95 ) {
			$advertise->fbid = $fbid;
			$advertise->logs = json_encode( $logs );
			$advertise->save( false );
